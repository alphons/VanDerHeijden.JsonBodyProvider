...
{
	await netproxyasync("./api/HelloWorld"); // Makes session (and cookie)
}
...
async function UnitTest()
{
	...
	r = await netproxyasync("./api/SimpleString3", { user: 'alphons' });
	C("SimpleString3", "r.user == 'alphons'");
	r = await netproxyasync("./api/SimpleString4", { user: 'alphons' });
	...
	r = await netproxyasync("./api/ArrayOfStrings2", ["admins", "editors", null, "sisters"]);
	...
	r = await netproxyasync("./api/ComplexSingleObject", { a: 'aaa', b: 'bbb' });
	...
	r = await netproxyasync("./api/ComplexArray", [{ a: 'a', b: null }, null, { a: 'c', b: 'd' }]);
